...
class PreguntaFormulario {
  ...
  // ── Paso 6: validación de una pregunta ───────────────────
  /// Retorna null si es válida, o el mensaje de error si no.
  String? validar() {
    if (pregunta.trim().isEmpty) return 'La pregunta no puede estar vacía';
    if (tipo != 'abierta' && opciones.length < 2) {
      return 'Debe tener al menos 2 opciones';
    }
    if (tipo != 'abierta' &&
        opciones.any((o) => o.trim().isEmpty)) {
...
class _FormularioBuilderState extends State<FormularioBuilder> {
  ...
  // ── Paso 6: validar todo el formulario ───────────────────
  /// Retorna true si es válido. Muestra errores si no.
  bool validar() {
    setState(() => _intentoGuardar = true);
    if (_preguntas.isEmpty) return false;
    return _preguntas.every((p) => p.validar() == null);
  }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
            ...
            children: [
              ...
              Expanded(
                child: Text(
                  ...
                  style:
                      TextStyle(fontSize: 11, color: Color(0xFF8B5CF6)),
                ),
              ),
            ],
          ),
        ...
        // Lista de preguntas
        ..._preguntas.asMap().entries.map(
              (entry) => _PreguntaCard(
                key: ValueKey(entry.key),
                index: entry.key,
                pregunta: entry.value,
                tipos: _tipos,
                mostrarError: _intentoGuardar,
                onEliminar: _preguntas.length > 1
                    ? () => _eliminarPregunta(entry.key)
                    : null,
                onChanged: () {
                  _notificar();
                  if (_intentoGuardar) setState(() {});
                },
              ),
            ),
...
class _PreguntaCardState extends State<_PreguntaCard> {
  ...
  @override
  Widget build(BuildContext context) {
    // ── Paso 6: calcular error de esta pregunta ───────────
    final error =
        widget.mostrarError ? widget.pregunta.validar() : null;
    final tieneError = error != null;
      ...
      child: Column(
        ...
        children: [
                ...
                child: DropdownButton<String>(
                  ...
                  onChanged: (v) {
                    setState(() {
                      ...
                      if (v != 'abierta' &&
                          widget.pregunta.opciones.isEmpty) {
                        widget.pregunta.opciones
                            .addAll(['Opción 1', 'Opción 2']);
                      }
                    });
                    widget.onChanged();
                  },
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(
                ...
                borderSide: BorderSide(
                  color: tieneError &&
                          widget.pregunta.pregunta.trim().isEmpty
                      ? const Color(0xFFFCA5A5)
                      : const Color(0xFFE5E7EB),
                ),
              ),
          ...
          if (widget.pregunta.tipo != 'abierta') ...[
            const SizedBox(height: 10),
            ...widget.pregunta.opciones.asMap().entries.map(
                  (e) => _OpcionRow(
                    ...
                    onChanged: (v) {
                      setState(
                          () => widget.pregunta.opciones[e.key] = v);
                      widget.onChanged();
                    },
            ...
            TextButton.icon(
              onPressed: () {
                setState(() => widget.pregunta.opciones.add(
                    'Opción ${widget.pregunta.opciones.length + 1}'));
                widget.onChanged();
              },
