...
class _EmpleadoDashboardState extends State<EmpleadoDashboard> {
  ...
  Future<void> _cargarDetalle(int idOnboarding) async {
    setState(() { _onboardingSeleccionado = idOnboarding; _loadingDetalle = true; _loadError = null; _stepsExpandidos.clear(); });
    try {
      ...
      for (final step in detalle.stepsConProgreso) {
        if (step.tasks.any((t) => !t.completada)) { _stepsExpandidos.add(step.idStep); break; }
      }
      setState(() { _detalle = detalle; _loadingDetalle = false; });
    } catch (e) {
      setState(() { _loadingDetalle = false; _loadError = e.toString().replaceAll('Exception: ', ''); });
    }
  }
  ...
  Future<void> _completarTask(int idTask) async {
    ...
    try {
      await ApiService.completarTask(idOnboarding: _onboardingSeleccionado!, idTask: idTask);
      await _cargarDetalle(_onboardingSeleccionado!);
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('¡Tarea completada!'), backgroundColor: Color(0xFF10B981), behavior: SnackBarBehavior.floating));
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Mi Onboarding', style: TextStyle(fontWeight: FontWeight.w600)),
        actions: [
          Padding(padding: const EdgeInsets.symmetric(horizontal: 8),
              child: Center(child: Text(auth.userName, style: const TextStyle(fontSize: 14)))),
          IconButton(icon: const Icon(Icons.refresh_rounded), tooltip: 'Actualizar', onPressed: _loadData),
          IconButton(icon: const Icon(Icons.logout_rounded), tooltip: 'Cerrar sesión',
  ...
  Widget _buildSinOnboarding() => Center(child: Padding(
    padding: const EdgeInsets.all(32),
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(width: 80, height: 80,
          decoration: BoxDecoration(color: const Color(0xFF1565C0).withOpacity(0.1), borderRadius: BorderRadius.circular(20)),
          child: const Icon(Icons.assignment_late_rounded, color: Color(0xFF1565C0), size: 40)),
      const SizedBox(height: 16),
  ...
  Widget _buildConOnboarding() {
    ...
    final colorEstado      = estado == 'COMPLETADO' ? const Color(0xFF10B981)
        : estado == 'EN_PROGRESO' ? const Color(0xFFF59E0B) : const Color(0xFF6B7280);
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_onboardings.length > 1) ...[
          OnboardingSelector(
            onboardings: _onboardings, seleccionado: _onboardingSeleccionado,
            onChanged: (id) { if (id != null && id != _onboardingSeleccionado) _cargarDetalle(id); },
          ),
          const SizedBox(height: 16),
        ],
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Expanded(child: Text(_detalle?.nombrePlan ?? 'Mi Plan',
                  style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold),
                  overflow: TextOverflow.ellipsis)),
              const SizedBox(width: 8),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: colorEstado.withOpacity(0.2),
                    borderRadius: BorderRadius.circular(6),
                    border: Border.all(color: colorEstado.withOpacity(0.4))),
                child: Text(estado, style: TextStyle(color: colorEstado, fontSize: 12, fontWeight: FontWeight.w600)),
              ),
            ]),
            if (_detalle?.fechaInicio != null) ...[
              const SizedBox(height: 4),
              Text('Inicio: ${_detalle!.fechaInicio!.toLocal().toString().split(' ')[0]}',
                  style: TextStyle(color: Colors.white.withOpacity(0.65), fontSize: 12)),
            ],
            const SizedBox(height: 20),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text('Progreso general', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 13)),
              Text('${progresoNum.toStringAsFixed(0)}%',
                  style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
            ]),
            const SizedBox(height: 8),
            ClipRRect(borderRadius: BorderRadius.circular(4),
              child: LinearProgressIndicator(value: progresoNum / 100,
        ...
        if (_loadError != null && _onboardings.isNotEmpty)
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text(_loadError!, style: const TextStyle(color: Color(0xFFDC2626), fontSize: 13))),
              TextButton(onPressed: () => _cargarDetalle(_onboardingSeleccionado!),
                  child: const Text('Reintentar', style: TextStyle(color: Color(0xFFDC2626), fontSize: 12))),
            ]),
          ),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          const Text('Etapas del plan',
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF1A1A2E))),
          if (_loadingDetalle) const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)),
        ]),
        ...
        if (_loadingDetalle && steps.isEmpty)
          ...
          Center(child: Padding(padding: const EdgeInsets.all(20),
            child: Text(_loadError != null ? 'No se pudieron cargar las etapas.' : 'Este plan no tiene etapas aún.',
